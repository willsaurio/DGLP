...
switch ($queryType) {
    ...
    case 'equipos':
        ...
        break;
    case 'nomina':
        $query = "SELECT `nombre_apellido` AS id,  `nombre_apellido` AS text, inss, cargo FROM `nomina` WHERE `estado` = 'ACTIVO' AND `nombre_apellido` LIKE '%$search%'";
        break;
    case 'centro_atencion':
        $query = "SELECT `centro` AS id, `centro` AS text FROM `centro_atencion` WHERE centro LIKE '%$search%'";
        break;
